...
.PHONY: lint check format clean build
...
test:  ## Run tests with pytest
	uv run pytest -rP -n auto --show-capture=no
...
test-parallel:  ## Run tests in parallel (alias for test)
	uv run pytest -rP -n auto

test-fast:  ## Run tests excluding slow ones
	uv run pytest -m "not slow" -rP -n auto

test-slow:  ## Run only slow tests
	uv run pytest -m slow -rP
...
setup: venv install-dev install-pre-commit  ## Complete setup for development
	...
	@echo "  make test-cov   - Coverage report"
